...
function QuestionList(props) {
  ...
  const ShowResult = (props,i) => {
	...
	let ShowResultParams = {'meetingId':null,'questionId':null,'question':props.question,'showresult':true};
  ...
  <div className="lg:text-center">
      ...
      <p className="mt-4 max-w-2xl text-xl leading-7 text-black lg:mx-auto">
        Release question one by one on voting time
      </p>
    </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead>
            <tr>
              ...
              <th className="px-10 py-3 app_theme_background text-left text-xl leading-4 font-medium text-white  tracking-wider">Action</th>
            </tr>
          </thead>
          ...
          {currentData && currentData .map((item,index) => (
            <tr key={index}>
              ...
              <td className="px-6 py-4 whitespace-no-wrap">
                <div className="text-sm leading-5 text-black text-lg">{item.question}</div>
              </td>
...
<div className="grid gap-4 grid-cols-2">
              <button disabled={disabledButtons[index]?'':''} onClick = {()=>releaseQuestion(item,index)} className={disabledButtons[index]?'group relative w-18 flex h-10 justify-center py-2 px-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-gray-600 focus:outline-none   transition duration-150 ease-in-out':'group relative w-18 flex h-10 justify-center py-2 px-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-green-600 focus:outline-none   transition duration-150 ease-in-out'} >
              {disabledButtons[index]?'Released':'Release'}
              </button>
			  <button  onClick = {()=>ShowResult(item,index)}
    ...
    <div className="flex justify-end">
    <button onClick = {()=>CompletePoll()} className="group relative w-24 flex justify-center py-2 px-2 border border-transparent h-10 text-sm leading-5 font-medium rounded-md text-white bg-green-600 hover:bg-green-500 focus:outline-none focus:border-green-700 focus:shadow-outline-green active:bg-green-700 transition duration-150 ease-in-out mr-8">
    <span>End Poll</span>
    </button>
    </div>

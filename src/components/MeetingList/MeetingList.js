...
import {AiOutlineEdit,AiFillEye,AiOutlineArrowUp,AiOutlineArrowDown,AiOutlineExport,AiOutlineDownload} from 'react-icons/ai';
 ...
 //   console.log('hideFeature',hideFeature);
  ...
  let history = useHistory();
   ...
   const [state , setState] = useState({
	...
	currentData = nameToSearch!==''?filteredData.slice(indexOfFirstData, indexOfLastData):data.slice(indexOfFirstData, indexOfLastData);
  ...
  const filterText = (e)=>{
		...
		let pages = [];	
		for (let i = 1; i <= Math.ceil(filtered_data.length / state.dataPerPage); i++) {
		pages.push(i);
		}
   ...
   const sortAscending = () => {	
	let sortedDataAsc;
	let field='m_name';	
	sortedDataAsc= data.sort((a, b) => (a[field] || "").toString().localeCompare((b[field] || "").toString()));   
  ...
  const sortDescending = () => {	  
	let sortedDataDesc;
	let field='m_name';
	sortedDataDesc= data.sort((a, b) => (b[field] || "").toString().localeCompare((a[field] || "").toString()))		  
  ...
  const sortAscendingDateTime = () => {	
	let sortedDataAsc;
	let field='m_date';		
	sortedDataAsc= data.sort((a,b) => moment(a[field]).diff(moment(b[field])));	
	setData(sortedDataAsc);
  ...
  const sortDescendingDateTime = () => {	  
	let sortedDataDesc;
	let field='m_date';	
	sortedDataDesc= data.sort((a,b) => moment(b[field]).diff(moment(a[field])));	
	setData(sortedDataDesc);
  ...
  const redirectToQuestionList = (props)=>{
    history.push('/questionlist/'+props.m_id+'/'+props.m_meeting_completed)
  }
  ...
  const redirectToExportList = (props)=>{
	  ...
	  let languageId =  parseInt(sessionStorage.getItem("languageId"));
		window.open(
		baseURL+'Meeting/download/report?meetingId='+props.m_id+'&languageId='+languageId,
		'_blank' 
		);
  }
  ...
  function ExportButton(props) {
    return <button onClick={()=>redirectToExportList(props.item)} className="group relative w-half flex justify-center py-2 px-4 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-green-600 hover:bg-green-500 focus:outline-none focus:border-green-700 focus:shadow-outline-green active:bg-green-700 transition duration-150 ease-in-out">{t('MeetingList.DownloadButton')}&nbsp; <AiOutlineDownload className="adjust-pos"/> </button> ;
  } 
  ...
  <div className="lg:text-center">
      ...
      <h3 className="lg:text-3xl leading-8 font-extrabold tracking-tight text-gray-900 sm:text-4xl sm:leading-10">
      {t('MeetingList.FormHeadingText')}
      </h3>
    </div>

  <div  className={hideFeature?'hidden':'flex justify-end mt-5'}>
  <button onClick={()=>redirectToAddMeeting()} className="bg-indigo-500 text-white  lg:mr-8 font-bold py-2 px-4  rounded inline-flex items-center">
  ...
  <span>{t('MeetingList.AddMeeting')}</span>
    ...
    <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
		...
		<input type="text" className="border-solid border-gray-400 border-2 p-3 md:text-xl w-full" id="Name" onChange={filterText} type="text" value={nameToSearch} placeholder="Search Meeting name" />	 
		</div>   
   ...
   <thead className="desktop">
                 <tr>
                   ...
                   <th className="px-6 py-3 app_theme_background text-left text-xl leading-4 font-medium text-white  tracking-wider">
                     {t('MeetingList.Name')} &nbsp;<AiOutlineArrowUp  className={ascendingOrder?'hidden':''} onClick={sortDescending} /> <AiOutlineArrowDown  className={!ascendingOrder?'hidden':''} onClick={sortAscending}/>
                   </th>
                   <th className="px-6 py-3 app_theme_background text-left text-xl leading-4 font-medium text-white  tracking-wider">
                     {t('MeetingList.Time')} &nbsp;<AiOutlineArrowUp  className={ascendingOrderDateTime?'hidden':''} onClick={sortDescendingDateTime} /> <AiOutlineArrowDown  className={!ascendingOrderDateTime?'hidden':''} onClick={sortAscendingDateTime}/>
                   </th>
			...
			{currentData && currentData.map((item,i) => (
				...
				<tr  key={i} className="flex flex-col flex-no wrap mt-4 sm:table-row ">
			...
			<td className="border-grey-light border hover:bg-gray-100 p-3"> {item.m_company}</td>
			<td className="border-grey-light border hover:bg-gray-100 p-3"> {item.m_link}</td>
					
			<td className="p-1 text-left text-white bg-gray-600 lg:hidden md:hidden">
			{t('MeetingList.Name')}
			</td>
			<td className="border-grey-light border hover:bg-gray-100 p-3"> {item.m_name}</td>

			
			<td className="p-1 text-left text-white bg-gray-600 lg:hidden md:hidden">
                {t('MeetingList.Time')}
              </td> 
					<td className="border-grey-light border hover:bg-gray-100 p-3"> {moment(item.m_date).format('DD-MM-YYYY HH:mm:ss')}</td>
			...
			<td className="p-1 text-left bg-gray-600 text-white lg:hidden md:hidden">
                {t('MeetingList.Questions')}
					...
					{item.m_questionsExist?<QuestionViewButton item ={item}/>:<QuestionUploadButton item ={item} />}</td>
					
		<td className="p-1 text-left bg-gray-600 text-white lg:hidden md:hidden">
                {t('MeetingList.Action')}
          </td>
					<td className={hideFeature?'hidden':'border-grey-light border hover:bg-gray-100 p-3'} >
					
					
					{item.m_meeting_completed?<ExportButton item ={item}/>:<button onClick={()=>redirectToEditMeeting(item)} className="group relative w-half flex justify-center py-2 px-4 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-green-600 hover:bg-green-500 focus:outline-none focus:border-green-700 focus:shadow-outline-green active:bg-green-700 transition duration-150 ease-in-out">{t('MeetingList.EditButton')} &nbsp;<span className="adjust-pos"><AiOutlineEdit /></span></button>}
	  ...
	  <ul className="flex pl-0 list-none rounded my-2 justify-center mt-5">
		...
		<li onClick={handlePrev} className={parseInt(state.currentPage)===1?'hidden':'relative hand-cursor block py-2 px-3 leading-tight bg-indigo-500 border border-gray-300 text-white border-r-0 ml-0 rounded-l hover:bg-indigo-600'} >Previous</li>

		 {pageNumbers && pageNumbers.map(number => (
		            <li className={parseInt(number)===parseInt(state.currentPage)?'relative hand-cursor block py-2 px-3 leading-tight border border-gray-300 text-white border-r-0 bg-gray-800':'relative hand-cursor block py-2 px-3 leading-tight bg-white border border-gray-300 text-blue-700 border-r-0'}  
              key={number}
              id={number}
			   onClick={handleClick}
            >
		...
		<li onClick={handleNext} className={parseInt(pageNumbers.length)===parseInt(state.currentPage)?'hidden':'relative hand-cursor block py-2 px-3 leading-tight bg-indigo-500 border border-gray-300 text-white border-r-0 ml-0 rounded-l hover:bg-indigo-600'}>Next</li>
	</ul>
    </div>
  </div>
</div>

...
function AddMeeting(props) {
  ...
  const [state , setState] = useState({
    ...
    const [Message,setMessage] = useState('Meeting created & login credentials send to meeting owner email successfully.');
...
// get userdata as formdata
        ...
        <div className="shadow sm:rounded-md sm:overflow-hidden">
          <div className="px-4 py-5 bg-white space-y-6 sm:p-6">
            <div className="grid grid-cols-3 gap-6">
              <div className="col-span-3 sm:col-span-2">
                ...
                <div className="mt-1 flex rounded-md shadow-sm">
                  <input type="text" id="Name" onChange={handleChange} className="h-10 border flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-black" value={state.Name} />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-1 flex rounded-md shadow-sm">
                <input type="text" id="MeetingOwner" onChange={handleChange} className="h-10 border flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-black" value={state.MeetingOwner} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-1 flex rounded-md shadow-sm">
                <input type="text" id="MeetingOwnerEmail" onChange={handleChange} className="h-10 border flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-black" value={state.MeetingOwnerEmail} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-1">
                <textarea id="Notes" rows="3" onChange={handleChange} className="shadow-sm h-20 border border-black mt-1 block w-full sm:text-sm
                rounded-md" value={state.Notes}></textarea>
              </div>
            ...
            <div>
              <label htmlFor="Meeting Time" className="block text-sm font-medium text-black">
                Meeting Date & Time
              </label>
              <div className="mt-1 flex rounded-md shadow-sm">
			  <DateTimePicker  onChange={onChangeDateTime}  value={MeetingDateTime} />
            ...
            <div>
              ...
              <input aria-label="CSV Upload" name="userdata" onChange={handleUserList} type="file"  className="appearance-none rounded-none relative block w-full px-3 py-2 border border-black text-black rounded-t-md focus:outline-none focus:shadow-outline-black focus:border-black-300 focus:z-10 sm:text-sm sm:leading-5"
                id="userdata" / >
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-black">
               ...
               <input aria-label="CSV Upload" name="question_list"   className="appearance-none rounded-none relative block w-full px-3 py-2 border border-black text-black rounded-t-md focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5"
                onChange={handleQuestionList} type="file" id="question_list" / >
            </div>
          </div>
          <div className="px-4 py-3 text-right sm:px-6">
            <button type="submit" className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-green-500  focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
              Save
            </button>
          </div>
        </div>
  ...
  <Modal Message={Message} open={ModalShow} Title={MeetingTitle} RedirectUrl="/meetinglist" />
</div>
